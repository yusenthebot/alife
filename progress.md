...
## Current state (Round 181 — 2026-06-21) — SPATIAL SIGNALLING, EMBODIED DIVISION OF LABOUR (POSITIVE, red-teamed): the learned channel now drives a REAL spatial harvest. R180 made the channel functional but its payoff was ABSTRACT — a correct decode paid energy directly; nobody had to GO anywhere. R181 grounds it in the 3D world (`signal_spatial`, default-off byte-identical; requires signal_task=True): the shared good-type `g_t∈{0,1}` now names WHICH of two fixed world PATCHES is ripe. SCOUTS (observe g_t) march to the true ripe patch; BLIND FORAGERS march to the patch their DECODED scout-signal names; the foraging reward is EARNED BY PHYSICAL PRESENCE at the ripe patch (`_signal_spatial_harvest`), not a bare decode. A forager only eats if it navigates to the place a scout's word points to. Money shot: the forager MASS tracks the flipping ripe patch IN SPACE. 禁止造假 — the figure shows it AND the honest seed-fragility.

**The contribution (`signal_spatial`, default-off byte-identical; requires signal_task=True).** Reuses the R180 urn machinery untouched; only the GAME becomes embodied. In `__init__`: two fixed patch centres separated along x by `patch_sep·size` (centred in y/z), harvest radius `patch_radius_frac·size`. In `_signal_task_decode`: when spatial, record `_spatial_steer` (scouts ∪ valid forager-listeners) + `_spatial_guess` (scouts→true g_t since they observe it, foragers→their urn decode `guess`), and ZERO the abstract decode reward (energy now comes from the harvest); urn reinforcement on correct decode is UNCHANGED (the learning is preserved). In `step`: `_spatial_steer_vel` drives steered agents straight at `_patches[their guessed patch]` at full per-agent speed (NO neighbour-position term → flocking impossible by construction); after the move `_signal_spatial_harvest` pays `signal_reward` to any forager within `_patch_r` of the TRUE ripe patch and records `_last_spatial_yield` = fraction of forager-listeners physically at the ripe patch. `signal_spatial=False` is byte-identical (no patches, no steering, R180 path intact — test-asserted). New config: `signal_spatial`/`patch_sep`/`patch_radius_frac`.

**HEADLINE — blind foragers physically navigate to the ripe patch a scout's signal names; the lr=0 control can't. Only signal_lr differs.** REAL-VERIFY (`scripts/run_genesis_r181.py` → `runs/r181_signal_spatial/signal_spatial.png`, EYE-VERIFIED, 11s): capacity 1200, n0 700, size 78, `task_flip=0.012` (slower than R180's 0.03 because physical transit between patches takes time the abstract decode did not), 1500 steps. LEARN (signal_lr=1.5), 3 seeds: tail spatial_yield **0.69 / 0.67 / 0.60** (mean 0.66), pop 1200; one-knob CONTROL (signal_lr=0, urns frozen) **0.25**; asymmetry **+0.40**; decode-accuracy probe 0.88 (learn) vs 0.50 (control). Panel A: 3 blue learn curves climb above 0.5, orange control flat at ~0.25. Panel B (the embodied money shot): forager x-position density HEATMAP over the full run with the green ripe-patch step line + cyan density-weighted MEAN forager-x overlaid — the cyan mean oscillates in lock-step with the green flips (transit lag) and bright density bands reach the high patch when green is high. Panel C: learn bars over the 0.7 line, control near chance.

**RED-TEAM CONFIRMED (independent skeptic agent; 4 refutations + a decisive SCRAMBLE null).** (a) FLOCKING impossible — `_spatial_steer_vel` targets `_patches[_spatial_guess]` using only the agent's OWN position, no neighbour term; the scramble null keeps scouts physically at the ripe patch 94% of the time yet forager yield still collapses, so a flocking path was AVAILABLE and NOT used. (b) NO LABEL LEAK — foragers' g_t input is masked to 0; their steering target is the urn `guess`, never `_good_type` (which is read only to SCORE who is at the ripe patch). (c) lr=0 is a clean one-knob — it gates ONLY the two Roth-Erev `np.add.at` lines; sampling/steering/harvest are bit-identical, same seed. (d) NOT SETTLING — a one-patch settler scores ~0.5; learn is 0.66 (ABOVE a settler, requires tracking the flip), control 0.25 (BELOW a settler — frozen urns resample a random target each step and thrash mid-transit). DECISIVE SCRAMBLE NULL: with learning ON but the scout's emitted symbol decoupled from the real ripe patch (random referent; harvest still real g_t), yield collapses 0.68→0.35 across seeds while scouts stay 94% at ripe — the SYMBOL's information, not scout proximity, is what gets foragers to the ripe patch. Fresh seed=5=0.78 reproduces (not one lucky seed).

**HONEST CAVEATS (recorded).** (i) The headline MEAN is SEED-FRAGILE (skeptic's seed1=0.453 vs 0.89/0.69 others) — the asymmetry-vs-control is robust on EVERY seed, but "~0.66" should be read as a mean over seeds, not a reliable per-run figure. (ii) `spatial_yield` is TRANSIT-CAPPED below 1.0 — even a perfect decoder is mid-transit a fraction of the time after each flip (this is why the absolute bar is the wrong yardstick; the principled verdict is the asymmetry vs the one-knob null). (iii) The EMERGENT part is the symbol→patch MAPPING (the learned urn convention); the LOCOMOTION is a hardwired go-to-target drive, NOT evolved — making the brain learn to act on the decoded symbol is a future rung (R182 option C). (iv) 2-patch/2-symbol binary (1 bit); the scramble residual 0.35 sits slightly above control 0.25 (residual decode ~0.60, a minor non-collapse, not a threat). 4 new tests (249 total: off-byte-identical, requires-signal_task, two-patch allocation, the embodied-harvest headline).

## Current state (Round 180 — 2026-06-21) — STAGE 2→3 BRIDGE, FUNCTIONAL SIGNALLING (POSITIVE, red-teamed): the learned channel now does real WORK — a DIVISION OF COGNITIVE LABOUR. R179 proved a Lewis convention EMERGES, but over a PRIVATE RANDOM referent bit — no shared world, no roles, "decode the bit" had no consequence beyond an abstract reward. R180 makes it functional: (a) a SHARED environmental good-type `g_t∈{0,1}` that FLIPS each step (prob `task_flip=0.03`) → the world is NON-STATIONARY, so no constant policy wins and communication is load-bearing every step; (b) a stable DIVISION OF LABOUR — each slot is a SCOUT (observes `g_t` → its referent input = g_t) or a BLIND FORAGER (g_t MASKED to 0 in its input), assigned by a deterministic golden-ratio low-discrepancy split (`scout_frac=0.5`, no RNG); (c) a forager can learn the current good-type ONLY by DECODING a scout's R179-urn signal, and a correct forager decode pays the foraging reward (real energy → birth/death). So a structurally-blind sub-population tracks a CHANGING world and EARNS only through others' signals = functional cooperation. 禁止造假 — the figure shows it AND the honest caveats.

**The contribution (`signal_task`, default-off byte-identical; requires signal_learn=True).** Same within-lifetime urn machinery as R179 — only the GAME changes. In `step`: when `signal_task`, the shared `_good_type` flips once per step (one RNG draw), then `ref = np.where(scout, g_t, 0)` (scouts observe g_t; foragers are blind). `_signal_task_decode` mirrors `_signal_learn_decode` but restricts the valid/reinforced pairs to `(nprox>0) & listener_forager & speaker_scout` — only blind foragers hearing informed scouts count, since foragers are the ones that need the info. A correct decode reinforces the forager's RECEIVER urn (symbol→g) and the scout's SENDER urn (g→symbol) +`signal_lr` (Roth-Erev, common-interest payoff). Records `_last_task_success` (forager decode accuracy on scout→forager pairs) + `_last_forager_decode` (mean decoded good-type, for the tracking overlay). `signal_task=False` is byte-identical (no flip draw, no role mask, no `_good_type`/`_is_scout` — test-asserted). New config: `signal_task`/`task_flip`/`scout_frac`.

**HEADLINE — blind foragers track a flipping world ONLY via scouts' learned signal; the lr=0 control can't. Only signal_lr differs.** REAL-VERIFY (`scripts/run_genesis_r180.py` → `runs/r180_signal_task/signal_task.png`, EYE-VERIFIED, 13s): capacity 1200, n0 700, size 75, 1600 steps. LEARN (signal_lr=1.5), 3 seeds: tail task_success **0.78 / 0.73 / 0.87** (mean 0.79), pop 1200; one-knob CONTROL (signal_lr=0, urns frozen) **0.50**; asymmetry **+0.30**. Panel A: 3 blue learn curves climb above 0.5 toward 0.7–0.9, orange control flat at chance. Panel B (the functional money shot): the foragers' aggregate decoded good-type (blue) TRACKS the true flipping `g_t` (black step) over the last 500 steps. Panel C: learn bars over the 0.7 functional line, control at the chance line.

**RED-TEAM CONFIRMED (the lr=0 control is the built-in null; it refutes two ways at once).** With frozen urns the IDENTICAL valid-pair selection scores 0.497 — which simultaneously proves (a) `g_t` is BALANCED ~50/50 (a constant guesser scores 0.5), so the learned 0.79 REQUIRES genuine symbol→g decoding, NOT constant-guessing or a degenerate skew; and (b) the forager-blind MASK holds — foragers carry ZERO g in their input (`np.where(scout, g, 0)`), so their only g-information is the decoded scout symbol; sever learning and they collapse to chance. Only scout→forager pairs are counted/reinforced (`speaker_scout & listener_forager`), so the success is not foragers reading each other. Robust 3/3 seeds. This is a GENUINE asymmetric division of cognitive labour — a shared NON-STATIONARY referent + a structurally-blind sub-population — materially different from R179's per-agent private-bit game, not a relabel.

**HONEST CAVEATS (recorded).** (i) Convention strength varies by seed (0.73–0.87, the same spread as R179's 0.65–0.83) and accuracy is capped below 1.0 by the re-learning transient after each flip (during which the other g-value's urn half may be stale) — partial pooling, not a perfect code. (ii) Canonical 2-symbol/2-state Lewis game; vocabulary/compositional scaling (Kirby iterated learning) is a future rung. (iii) The payoff is the abstract foraging REWARD folded into energy, not yet a SPATIAL harvest (scouts don't yet point foragers to a physical patch) — coupling the channel to real movement/harvest is R181 option (A). 4 new tests (245 total: off-byte-identical, requires-signal_learn, role-assignment, the division-of-labour headline).
...
**Current ceiling (post-R181): the learned channel is now EMBODIED — it drives a REAL spatial harvest. Stage-3
division of labour landed visibly in the 3D world.** R179 cleared Stage-2 (a Lewis convention emerges); R180 made it
functional (foragers track a flipping bit); R181 made it SPATIAL: `g_t` names which of two world PATCHES is ripe,
blind foragers physically NAVIGATE to the patch a scout's signal names and only eat by ARRIVING there (`signal_spatial`).
REAL-VERIFY + independent red-team CONFIRMED (scramble null severs the channel and the behaviour dies): 3-seed learn
tail spatial_yield 0.69/0.67/0.60 (mean 0.66) vs the one-knob lr=0 control 0.25 (asymmetry +0.40); decode 0.88 vs 0.50.
The new ceiling: the embodied channel is still (a) a FIXED golden-ratio scout/forager split, not evolved; (b) a
HARDWIRED go-to-target locomotion (the symbol→patch MAPPING emerges, but acting on it is scripted, not learned by the
brain); (c) a 2-patch/2-symbol code (1 bit). **Three live directions, ranked by ambition×feasibility:**
(A) **SCALE TO N>2 PATCHES / larger vocabulary + compositionality (Kirby iterated learning).** Move from 1 bit to K
referents + a transmission bottleneck across generations → the signal must carry >1 bit and pressure builds for a
COMPOSITIONAL code. Highest-ambition language result; the spatial harvest makes it concrete (not a standalone toy).
(B) **EVOLVE THE ROLES (who becomes a scout) — Stage-3 proper.** Replace the fixed split with a heritable scout/forager
disposition under selection (needs a scouting COST so an interior mix is an equilibrium) → the division of labour
itself EVOLVES rather than being imposed.
(C) **MAKE LOCOMOTION EVOLVED, not hardwired.** Feed the decoded symbol into the brain and let the NN learn to act on
it (approach the named patch) under selection → closes the last scripted gap; the whole sense→signal→decode→act loop
is then learned. Cleanest "no scripted theatre" win but lower headline-novelty than (A).
**Bias: (A) next — scaling the vocabulary on top of the now-EMBODIED channel is the genuine leap toward language, and
the spatial harvest grounds it. Decide at ORIENT. Do NOT re-run the R181 verify (it is eye-verified + red-teamed).**
...
(3) **STAGE (2) EMERGENT SIGNALLING** — pivot off frontier 1 onto the next ladder rung: emergent signalling →
language from scratch, seeded by the R142 diet specialists / R156 divergent traditions.
**Bias: the next LEAP IN KIND is (1) — the body now follows the culture; make the CULTURE'S deepening itself
non-asymptotic so the whole embodied+cultural complexity keeps climbing without an asymptote.**
