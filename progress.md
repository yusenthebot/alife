...
**Current ceiling (post-R180): the learned channel is now FUNCTIONAL — a DIVISION OF COGNITIVE LABOUR. Stage 2→3
bridge crossed.** R179 cleared Stage-2 (a Lewis convention emerges); R180 made it DO WORK: a shared NON-STATIONARY
good-type `g_t` flips each step, a structurally-BLIND forager sub-population tracks the changing world ONLY by
decoding informed SCOUTS' learned signals (`signal_task`). REAL-VERIFY + red-team CONFIRMED: 3-seed learn tail
task_success 0.78/0.73/0.87 (mean 0.79) vs the one-knob lr=0 control 0.50 (asymmetry +0.30); the lr=0 null
simultaneously proves g is balanced (so 0.79 needs real decoding) AND the forager mask holds. The new ceiling:
the functional channel is (a) ABSTRACT — its payoff is a reward folded into energy, NOT a spatial harvest (scouts
don't yet point foragers to a physical food patch); (b) roles are a FIXED golden-ratio split, not evolved; (c)
still a 2-symbol/2-state code. **Three live directions, ranked by ambition×feasibility:**
(A) **MAKE IT SPATIAL — couple the channel to a REAL harvest (TOP — closes the loop from signal to physical act).**
N food patches/types, scouts sense which is currently ripe and signal it, foragers DECODE and MOVE to that patch and
harvest real energy there. Emergence test: per-capita harvest is higher with the learned channel than a deaf/scrambled
control, and rises as the convention forms. Turns "foragers track a bit" into "foragers eat better because they
listened" — the genuine Stage-3 division of labour in the 3D world.
(B) **EVOLVE THE ROLES (who becomes a scout).** Replace the fixed split with a heritable scout/forager disposition
under selection → the division of labour itself EVOLVES (some lineages specialise as informants). Stage-3 proper.
(C) **SCALE THE VOCABULARY / compositionality (Kirby iterated learning).** 2→K referents + a transmission bottleneck
across generations → pressure for a COMPOSITIONAL code. Highest-ambition language result but risks a standalone toy.
**Bias: (A) next — it makes the functional channel act on the PHYSICAL world (real harvest, not an abstract reward)
and lands the division of labour visibly in the 3D sim. Decide at ORIENT. Do NOT re-run the R180 verify.**
